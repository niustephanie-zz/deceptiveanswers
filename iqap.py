...
class Item:
    ...
    def majority_label(self):
        ...
        max_label, max_count = sorted(self.response_counts().items(), key=itemgetter(1), reverse=True)[0]
        if max_count <= 15:
            max_label = None
        return max_label
    ...
    def featurize(self):
        ...
        a = self.Answer
        features = []
        if self.Prefix.lower() == "yes" or self.Prefix.lower() == "no":
            a = self.Prefix + " " + self.Answer
            features.append(self.Prefix)

        splitQ = q.split(" ")
        splitA = a.split(" ")
        features.append(len(splitQ))
        features.append(len(splitA))
        features.append(splitA[0])
        features.append(splitA[len(splitA) - 1])
        features.append(q)
        features.append(a)
